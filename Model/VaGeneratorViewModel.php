...
class CMODEL_VA_GENERATOR_VIEW extends CMODEL_VA_GENERATOR {
  ...
  function get_template_template_filename() {
    return $this->_template_template_filename;
  }

  function get_view_directory() {
    return $this->_app_dir . "views/" . $this->_lower_controller . "/";
  }
  function get_view_filename() {
    return $this->_lower_task . "_view.inc";
  }
  function get_view_file() {
    return $this->get_view_directory() . $this->get_view_filename();
  }

  function get_view_template_directory() {
    return $this->_app_dir . "templates/" . $this->_lower_controller . "/";
  }
  function get_view_template_filename() {
    return $this->_lower_task . ".php";
  }
  function get_view_template_file() {
    return $this->get_view_template_directory() . $this->get_view_template_filename();
  }

  function has_view_file() {
    return is_file($this->get_view_file());
  }
  function has_view_template_file() {
    return is_file($this->get_view_template_file());
  }
  function create_cmodel() {
    return parent::create_cmodel($this->_model);
  }

  function set_has_form($has_form) {
    $this->_has_form = $has_form;
  }
  function set_page_title($page_title) {
    $this->_page_title = $page_title;
  }
  function set_view_type($view_type) {
